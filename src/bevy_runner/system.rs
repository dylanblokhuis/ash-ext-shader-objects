...
use super::{
    ...
    vulkan_windows::{
        attempt_grab, get_best_videomode, get_fitting_videomode, winit_window_position,
        BevyVulkanoWindows, VulkanContext,
    },
};
...
pub(crate) fn create_window<'a>(
    ...
    context: Res<VulkanContext>,
    settings: NonSend<VulkanSettings>,
) {
    for (entity, mut window) in created_windows {
        ...
        let vulkano_window =
            vulkano_windows.create_window(event_loop, entity, &window, &context, &settings);
        window
            .resolution
