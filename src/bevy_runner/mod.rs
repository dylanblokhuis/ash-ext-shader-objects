...
use crate::bevy_runner::{
    config::UpdateMode,
    system::{create_window, CachedWindow},
};

use self::{
    config::VulkanSettings,
    system::{changed_window, despawn_window},
    vulkan_windows::{BevyVulkanoWindows, VulkanContext},
};

#[derive(Default)]
pub struct VulkanWinitPlugin;

impl Plugin for VulkanWinitPlugin {
    fn build(&self, app: &mut App) {
        ...
        let vulkan_config = VulkanSettings {
            ..Default::default()
        };

        app.init_non_send_resource::<BevyVulkanoWindows>()
            .insert_resource(vulkan_context)
            .insert_non_send_resource(vulkan_config)
...
pub fn winit_runner(mut app: App) {
    ...
    let return_from_run = app
        .world
        .non_send_resource::<VulkanSettings>()
        .return_from_run;

    trace!("Entering winit event loop");

    let mut focused_window_state: SystemState<(NonSend<VulkanSettings>, Query<&Window>)> =
