...
use crate::{bevy_runner::converters::convert_window_level, ctx::ExampleBase};

use super::config::VulkanSettings;
...
impl BevyVulkanoWindows {
    pub fn create_window(
        ...
        entity: Entity,
        window: &Window,
        _vulkano_context: &VulkanContext,
        _settings: &VulkanSettings,
    ) -> &VulkanoWindow {
        ...
        let vulkano_window = {
            let window_renderer = ExampleBase::new(winit_window, window.present_mode);

            VulkanoWindow {
                renderer: Arc::new(window_renderer),
            }
        };
